<template>
  <div class="app-container">
    <div>
      <el-button type="primary" style="float:left" @click="handleAdd">新增权限</el-button>
      <el-input v-model="searchdata.perm_name" style="width: 20%;float: right;margin-right: 5%" placeholder="请输入搜索权限" prefix-icon="el-icon-search" @change="loadPerm" />
    </div>
    <el-table
    ...
    >
      ...
      <el-table-column label="权限id" property="perm_id" />
    ...
    <el-dialog :title="title" :visible.sync="addVisible">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="confirmPerm">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
<script>
import { getPerms_by_filterdata, addPerm, editPerm } from '@/api/perm'
export default {
  name:'PermList',
  data() {
    return {
  ...
  methods: {
    loadPerm() {
      ...
      this.searchdata.perm_name === '' ? this.searchdata.perm_name = null : this.searchdata
      getPerms_by_filterdata(this.pagination.currentPage, this.pagination.pageSize, this.searchdata).then(response => {
